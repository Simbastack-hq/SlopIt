...
| 13 | **Stale language homes are pruned last.** `MutationRenderer.pruneLanguageHomes(blogId)`: `readdir(<blogDir>/lang)`, remove every subdirectory not in the current non-root language set, remove `lang/` itself when empty; ENOENT-tolerant. Called as the **final** step of every mutation's render sequence (`createPost`, both published branches of `updatePost`, `deletePost`, `updateBlog`), after every write has succeeded. | Deleting inside `renderBlog` would run before manifests and sibling renders; a later failure would compensate the DB but leave `/lang/de/` gone (Codex #2). Same destructive-last contract as `removePostFiles`. Exact, no heuristics: `lang/` holds only what we wrote. Calling it at every site, not only "shrink" sites, means nobody has to reason about which mutations shrink (a `language` patch on a published post does). |
...
```ts
translationOf: z.string().min(2).max(100)
...
`.translations` in `style.css`: a wrapping inline row of muted small links separated by `·` (`::before` on every child after the first), logical margins, `aria-current` in `--text`. Same visual weight as `<time>`. Roughly 15 lines; RTL-safe via logical properties like the rest of the theme.

### Chrome strings

`ThemeStrings` gains `otherLanguages` ("Other languages", "Andere Sprachen", "Autres langues", "Otros idiomas", "Outros idiomas", "Altre lingue", "Другие языки", "他の言語", "其他语言", "لغات أخرى", "अन्य भाषाएँ"). The drift-guard test refuses a half-translated entry, as today.
