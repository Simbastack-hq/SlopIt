...
| 10 | All four file types are emitted by **pure string builders** in `src/rendering/frontmatter.ts` (`.md`) and `src/rendering/feeds.ts` (`llms.txt`, `feed.xml`, `sitemap.xml`). **No new theme template files.** The existing `render(template, vars)` machinery handles `post.html` + `index.html` and has no loop support — needed by RSS/sitemap/llms.txt — so dragging it into per-item iteration would be a worse fit than `Array.map().join('\n')` in the builder. The pure-builder approach also keeps `ThemeAssets` unchanged (current type only loads `post.html`, `index.html`, `style.css`), avoiding interface growth for no benefit. | Earlier drafts proposed `.template` files for all four; the reviewer caught the inconsistency — the plan implements pure builders. Aligning the spec to the plan: builders win. |
...
| `src/posts.ts` | MODIFY | The published→draft transition inside `updatePost` (`src/posts.ts:376`) and `deletePost` (`src/posts.ts:519`) invoke the cleanup methods on the renderer in this order: `renderManifests` first, destructive per-post file ops (`removePostFiles`, `deletePostMarkdown`) last — so a manifest failure can't strand the DB compensated to `published` with the per-post files already gone. There is no `unpublishPost` function in core; the unpublish flow is `updatePost(blogId, slug, { status: 'draft' })`. |
