...
5. **No cookie, no IP.** The hosted root negotiates from `Accept-Language` (exact tag, else same language + same maximized script) and honours `?lang=` as "explicit choice, don't redirect". The privacy policy promises no cookies on rendered blog pages, so an explicit choice lasts for the click, by design.
