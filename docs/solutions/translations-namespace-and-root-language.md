...
3. **Stale homes are pruned last, at every mutation site, directories only.** Compensation after a failed write runs every recovery step independently (`attemptAll`) and reports an incomplete recovery in the thrown error (`rethrowWithRecovery`); `deletePost` removes files and prunes even when its re-render fails, because the row is already gone and no retry can reach them. `lang/` exists only while the blog publishes in two or more languages (deliberate; see the spec amendments). `Renderer.pruneLanguageHomes` runs after every render in `createPost`, `updatePost`, `deletePost` and `updateBlog` — after, because it is destructive and a render failure must leave the old home for the compensated DB state; every site, because of rule 2; directories only, because a pre-reservation post with slug `lang` owns `lang/index.html` (a file) and the `lang.md` guard in `renderBlog` only fires on multilingual blogs.
